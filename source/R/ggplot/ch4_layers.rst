Ch4. Build a plot layer by layer
""""""""""""""""""""""""""""""""
The source R script available :download:`here <ch4_layers.R>`
...
You've already created layers with functions like ``geom_point()`` and
``geom_histogram()``. In this chapter, you'll dive into the details of a
layer, and how you can control all five components: data, the aesthetic
mappings, the geom, stat, and position adjustments. The goal here is to
give you the tools to build sophisticated plots tailored to the problem
at hand.

Building a plot
---------------

So far, whenever we've created a plot with ``ggplot()``, we've
...
``geom_point()`` is a shortcut. Behind the scenes it calls the
``layer()`` function to create a new layer: :raw-latex:`\indexf{layer}`
...
:raw-latex:`\index{Layers!components}`

-  **mapping**: A set of aesthetic mappings, specified using the
   ``aes()`` function and combined with the plot defaults as described
   in `aesthetic mappings <#sec:aes>`__. If ``NULL``, uses the default
   mapping set in ``ggplot()``.

-  **data**: A dataset which overrides the default plot dataset. It is
   usually omitted (set to ``NULL``), in which case the layer will use
   the default data specified in ``ggplot()``. The requirements for data
   are explained in more detail in `data <#sec:data>`__.

-  **geom**: The name of the geometric object to use to draw each
   observation. Geoms are discussed in more detail in
   `geom <#sec:data>`__, and `the toolbox <#cha:toolbox>`__ explores
   their use in more depth.
   ...
   plot, as described in `setting vs. mapping <#sub:setting-mapping>`__.
   You can pass params in ``...`` (in which case stat and geom
   parameters are automatically teased apart), or in a list passed to
   ``geom_params``.

-  **stat**: The name of the statistical tranformation to use. A
   ...
   the "identity" stat. Learn more in `statistical
   transformations <#sec:stat>`__.
   ...
   (in which case stat and geom parameters are automatically teased
...
-  **position**: The method used to adjust overlapping objects, like
   ...
   `position <#sec:position>`__.

It's useful to understand the ``layer()`` function so you have a better
mental model of the layer object. But you'll rarely use the full
``layer()`` call because it's so verbose. Instead, you'll use the
shortcut ``geom_`` functions: ``geom_point(mapping, data, ...)`` is
exactly equivalent to ``layer(mapping, data, geom = "point", ...)``.

Data
----

Every layer must have some data associated with it, and that data must
be in a tidy data frame. You'll learn about tidy data in `tidy
data <#cha:data>`__, but for now, all you need to know is that a tidy
data frame has variables in the columns and observations in the rows.
This is a strong restriction, but there are good reasons for it:
:raw-latex:`\index{Data}` :raw-latex:`\indexf{data.frame}`
...
-  It enforces a clean separation of concerns: ggplot2 turns data frames
   ...
   format (learn more about that in `model
   visualisation <#sub:modelvis>`__).
...
Note that you need the explicit ``data =`` in the layers, but not in the
call to ``ggplot()``. That's because the argument order is different.
This is a little inconsistent, but it reduces typing for the common case
where you specify the data once in ``ggplot()`` and modify aesthetics in
each layer.
...
arguments to ``ggplot()`` are ordered, it actually requires more
...
1. The first two arguments to ggplot are ``data`` and ``mapping``. The
   first two arguments to all layer functions are ``mapping`` and
   ``data``. Why does the order of the arguments differ? (Hint: think
   about what you set most commonly.)

2. The following code uses dplyr to generate some summary statistics
   about each class of car (you'll learn how it works in `data
   transformation <#cha:dplyr>`__).
...
The aesthetic mappings, defined with ``aes()``, describe how variables
are mapped to visual properties or **aesthetics**. ``aes()`` takes a
sequence of aesthetic-variable pairs like this:
:raw-latex:`\index{Aesthetics!mapping}` :raw-latex:`\indexf{aes}`
...
(If you're American, you can use *color*, and behind the scenes ggplot2
will correct your spelling ;)

Here we map x-position to ``displ``, y-position to ``hwy``, and colour
to ``class``. The names for the first two arguments can be omitted, in
which case they correspond to the x and y variables. That makes this
specification equivalent to the one above:
...
While you can do data manipulation in ``aes()``, e.g.
``aes(log(carat), log(price))``, it's best to only do simple
calculations. It's better to move complex transformations out of the
``aes()`` call and into an explicit ``dplyr::mutate()`` call, as you'll
learn about in `mutate <#mutate>`__. This makes it easier to check your
work and it's often faster because you need only do the transformation
once, not every time the plot is drawn.

Never refer to a variable with ``$`` (e.g., ``diamonds$carat``) in
``aes()``. This breaks containment, so that the plot no longer contains
everything it needs, and causes problems if ggplot2 changes the order of
the rows, as it does when facetting. :raw-latex:`\indexc{\$}`

Specifying the aesthetics in the plot vs. in the layers
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Aesthetic mappings can be supplied in the initial ``ggplot()`` call, in
individual layers, or in some combination of both. All of these calls
create the same plot specification:
:raw-latex:`\index{Aesthetics!plot vs. layer}`
...
to a *single* value by specifying it in the layer parameters. We **map**
an aesthetic to a variable (e.g., ``aes(colour = cut)``) or **set** it
to a constant (e.g., ``colour = "red"``). If you want appearance to be
governed by a variable, put the specification inside ``aes()``; if you
want override the default size or colour, put the value outside of
``aes()``. :raw-latex:`\index{Aesthetics!setting}`

The following plots are created with similar code, but have rather
different outputs. The second plot **maps** (not sets) the colour to the
value 'darkblue'. This effectively creates a new variable containing
only the value 'darkblue' and then scales it with a colour scale.
Because this value is discrete, the default colour scale uses evenly
spaced colours on the colour wheel, and since there is only one value
this colour is pinkish.
...
colours. You'll learn more about that in `the identity
scale <#sub:scale-identity>`__.
...
Geometric objects, or **geoms** for short, perform the actual rendering
of the layer, controlling the type of plot that you create. For example,
using a point geom will create a scatterplot, while using a line geom
will create a line plot.

-  Graphical primitives:

   -  ``geom_blank()``: display nothing. Most useful for adjusting axes
      limits using data.
   -  ``geom_point()``: points.
   -  ``geom_path()``: paths.
   -  ``geom_ribbon()``: ribbons, a path with vertical thickness.
   -  ``geom_segment()``: a line segment, specified by start and end
      position.
...
-  One variable:
   ...
   -  Continuous
      ...
      -  ``geom_histogram()``: bin and count continuous variable,
         display with bars.
...
-  Two variables:
   ...
   -  One continuous, one discrete:
      ...
      -  ``geom_bar(stat = "identity")``: a bar chart of precomputed
         summaries.
   ...
   -  Spatial
      ...
      -  ``geom_map()``: fast version of ``geom_polygon()`` for map
         data.
...
-  Three variables:
   ...
   -  ``geom_raster()``: fast version of ``geom_tile()`` for equal sized
      tiles.

Each geom has a set of aesthetics that it understands, some of which
*must* be provided. For example, the point geoms requires x and y
position, and understands colour, size and shape aesthetics. A bar
requires height (``ymax``), and understands width, border colour and
fill colour. Each geom lists its aesthetics in the documentation.

Some geoms differ primarily in the way that they are parameterised. For
example, you can draw a square in three ways:
:raw-latex:`\index{Geoms!parameterisation}`

-  By giving ``geom_tile()`` the location (``x`` and ``y``) and
   dimensions (``width`` and ``height``).
   :raw-latex:`\indexf{geom\_tile}`

-  By giving ``geom_rect()`` top (``ymax``), bottom (``ymin``), left
   (``xmin``) and right (``xmax``) positions.
   :raw-latex:`\indexf{geom\_rect}`

-  By giving ``geom_polygon()`` a four row data frame with the ``x`` and
   ``y`` positions of each corner.
...
1. Download and print out the ggplot2 cheatsheet from
   http://www.rstudio.com/resources/cheatsheets/ so you have a handy
   visual reference for all the geoms.
...
3. What's the best way to master an unfamiliar geom? List three
...
-  ``stat_bin()``: ``geom_bar()``, ``geom_freqpoly()``,
   ``geom_histogram()``
...
-  ``stat_summary2d()``, ``stat_summary_hex()``: summarise binned
   values.
...
``stat_()`` function and override the default geom, or add a ``geom_()``
function and override the default stat:
...
``stat_bin``, the statistic used to make histograms, produces the
following variables: :raw-latex:`\index{Stats!creating new variables}`
:raw-latex:`\indexf{stat\_bin}`

-  ``count``, the number of observations in each bin
-  ``density``, the density of observations in each bin (percentage of
   total / bar width)
...
the height of the bars to the number of observations (``count``), but if
you'd prefer a more traditional histogram, you can use the density
(``density``). To refer to a generated variable like density, "``..``\ "
must surround the name. This prevents confusion in case the original
dataset includes a variable with the same name as a generated variable,
and it makes it clear to any later reader of the code that this variable
was generated by a stat. Each statistic lists the variables that it
creates in its documentation. :raw-latex:`\indexc{..}` Compare the
y-axes on these two plots:
...
example, it's hard to compare the distribution of ``price`` within
``cut`` because some groups are quite small. It's easier to compare if
we standardise each group to take up the same area:
...
3. Read the help for ``stat_sum()`` then use ``geom_count()`` to create
   a plot that shows the proportion of cars that have each combination
   of ``drv`` and ``trans``.

Position adjustments
--------------------

:raw-latex:`\index{Position adjustments}`
...
:raw-latex:`\index{Dodging}`
:raw-latex:`\index{Side-by-side|see{Dodging}}`
:raw-latex:`\indexf{position\_dodge}` :raw-latex:`\index{Stacking}`
:raw-latex:`\indexf{position\_stack}`
:raw-latex:`\indexf{position\_fill}`

-  ``position_stack()``: stack overlapping bars (or areas) on top of
   each other.
-  ``position_fill()``: stack overlapping bars, scaling so the top is
   always at 1.
-  ``position_dodge()``: place overlapping bars (or boxplots)
   side-by-side.
...
``position_identity()``. The identity position adjustment isn't useful
for bars, because each bar obscures the bars behind, but there are many
geoms that don't need adjusting, like lines:
...
-  ``position_jitterdodge()``: dodge points within groups, then add a
   little random noise.

:raw-latex:`\indexf{position\_nudge}`
:raw-latex:`\indexf{position\_jitter}`
:raw-latex:`\indexf{position\_jitterdodge}`
...
you construct a position adjustment object, supplying additional
...
This is rather verbose, so ``geom_jitter()`` provides a convenient
shortcut:
...
2. Many position adjustments can only be used with a few geoms. For
...
3. Why might you use ``geom_jitter()`` instead of ``geom_count()``? What
...
.. |image0| image:: _figures/layers/layer1-1.png
.. |image1| image:: _figures/layers/unnamed-chunk-1-1.png
.. |image2| image:: _figures/layers/layer2-1.png
.. |image3| image:: _figures/layers/unnamed-chunk-2-1.png
.. |image4| image:: _figures/layers/unnamed-chunk-3-1.png
.. |image5| image:: _figures/layers/unnamed-chunk-5-1.png
.. |image6| image:: _figures/layers/unnamed-chunk-6-1.png
.. |image7| image:: _figures/layers/unnamed-chunk-6-2.png
.. |image8| image:: _figures/layers/unnamed-chunk-6-3.png
.. |image9| image:: _figures/layers/unnamed-chunk-6-4.png
